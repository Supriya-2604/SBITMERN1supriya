...
function StudentList() {
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get("https://sbitmern1supriya-server.onrender.com/api/students");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://sbitmern1supriya-server.onrender.com/api/students/${id}`);
      await fetchStudents();
    } catch (error) {
  ...
  const handleEditSave = async () => {
    try {
      const response = await axios.put(
        `https://sbitmern1supriya-server.onrender.com/api/students/${selectedStudent._id}`,
        selectedStudent
      );
